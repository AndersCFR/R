...
probabilidad_visita_sitio <- 0.983  # Probabilidad de comprar shampoo natural
probabilidad_registro <- 0.204      # Probabilidad de fidalización de la marca
probabilidad_compra <- 0.192   # Probabilidad de compra 
```
...
for (visitante in 1:visitantes_iniciales) {
  ...
  if (runif(1) <= probabilidad_visita_sitio) {
    ...
    if (runif(1) <= probabilidad_registro) {
      ...
      # Etapa 3: Inscripción después de registrarse
      if (runif(1) <= probabilidad_compra) {
        inscripciones[visitante] <- 1
      }
    }
  }
...
# Imprimir resultados
cat("Número total de visitantes a la tienda:", visitantes_iniciales, "\n")
cat("Número de visitantes que visitaron el sitio web de la campana:", total_visitas, "\n")
cat("Número de visitantes que se registraron después de visitar el sitio:", total_registros, "\n")
cat("Número de visitantes que compraron el producto despues de registrarse:", total_inscripciones, "\n")


```

en el caso de arriba se puede observar que en la muestra de 3000 para la compra de un producto en el supermercado, 2448 visitaron el sitio web de la campana, de los cuales 716 se registraron despues de vsisitar y finalmente de esos, unos 242 compraron el producto
...
probabilidad_acum_inscripcion <- probabilidad_acum_registro * probabilidad_compra
...
cat("Probabilidad acumulativa de compra:", probabilidad_acum_inscripcion, "\n")

```

Respuesta: 

Se puede abservar que el 98,3% visitaron el sitio web y el producto, el 20% se registro y tan solo el 3% lo compra a la final
...
Se puede observarque las visitas es mas del doble y los registtrso es una tercera parte aproximadamenre y las inscripciones es la mitad de resgitrsos.
...
```{r}
tasa_conversion_ver_tomar <- total_tomar / total_ver
tasa_conversion_tomar_comprar <- total_compra / total_tomar

cat("Tasa de conversión de ver a tomar:", tasa_conversion_ver_tomar, "\n")
cat("Tasa de conversión de tomar a comprar:", tasa_conversion_tomar_comprar, "\n")
...
la campaña numero B tiene una tasa de conversion superior, ya que en corelacion hay menos personas que visitaron el sitio web que la A, pero muchas mas personas se registran en el B que en el A. 
...
tasa_conversion_nueva <- 0.35  # Supongamos que estamos probando una nueva versión de la página de registro
...
```{r}
prom <- mean(datos_MushucRuna$Edad)
promedio <- as.integer(prom) #as.integer hace que la edad se un valor entero
print(paste("Promedio de las edades es:", promedio))

mediana <-median(datos_MushucRuna$Edad)
print(paste("La mediana de las edades es:", mediana))
...
tabla_retiro <- table(datos_MushucRuna$Retiro_Efectivo)
probabilidades_retiro <- prop.table(tabla_retiro)

tabla_deposito <- table(datos_MushucRuna$Deposito_Efectivo)
probabilidades_deposito <- prop.table(tabla_deposito)

tabla_transferencia <- table(datos_MushucRuna$Transferencia_Bancaria)
probabilidades_transferencia <- prop.table(tabla_transferencia)
print(probabilidades_retiro)
print(probabilidades_deposito)
print(probabilidades_transferencia)
...
print(prob_retiro_deposito)
print(prob_retiro_o_transferencia)
```
...
```{r}
correlacion_retiro <- cor(datos_MushucRuna$Edad, datos_MushucRuna$Retiro_Efectivo, method = "pearson")
correlacion_deposito <- cor(datos_MushucRuna$Edad, datos_MushucRuna$Deposito_Efectivo, method = "pearson")
correlacion_transferencia <- cor(datos_MushucRuna$Edad, datos_MushucRuna$Transferencia_Bancaria, method = "pearson")
print(correlacion_retiro)
print(correlacion_deposito)
print(correlacion_transferencia)
```
...
barplot(probabilidades_retiro, main = "Probabilidades de Retiro Efectivo", xlab = "Tipo de Transacción", ylab = "Probabilidad", col = "blue")
barplot(probabilidades_deposito, main = "Probabilidades de Deposito Efectivo", xlab = "Tipo de Transacción", ylab = "Probabilidad", col = "green")
barplot(probabilidades_transferencia, main = "Probabilidades de Transferencia Bancaria", xlab = "Tipo de Transacción", ylab = "Probabilidad", col = "red")
...
```{r}
datos_MushucRuna$Grupo_Edad <- cut(datos_MushucRuna$Edad, breaks = c(10, 20, 30, 40, 50), include.lowest = TRUE)

Grupo_retiro <- table(datos_MushucRuna$Grupo_Edad, datos_MushucRuna$Retiro_Efectivo)
probabilidades_retiro_grupo <- prop.table(Grupo_retiro, margin = 1)

Grupo_deposito <- table(datos_MushucRuna$Grupo_Edad, datos_MushucRuna$Deposito_Efectivo)
probabilidades_deposito_grupo <- prop.table(Grupo_deposito, margin = 1)

Grupo_transferencia <- table(datos_MushucRuna$Grupo_Edad, datos_MushucRuna$Transferencia_Bancaria)
probabilidades_transferencia_grupo <- prop.table(Grupo_transferencia, margin = 1)

print(probabilidades_retiro_grupo)
print(probabilidades_deposito_grupo)
print(probabilidades_transferencia_grupo)
